...
enum class TransactionType(val description: String) {
    ...
    fun getRegex(): Regex {
        return when (this) {
            ...
            SEND_MONEY -> "(?<txnId1>\\b[A-Z0-9]+\\b) Confirmed\\. Ksh(?<amount1>[\\d,]+\\.?\\d{0,2}) sent to (?<recipient1>[A-Za-z0-9\\s\\p{P}\\p{S}_]+) (?<phone1>(?:\\+254|0)?\\d{9}) on (?<date1>\\d{1,2}/\\d{1,2}/\\d{2}) at (?<time1>\\d{1,2}:\\d{2} [APM]{2})\\. New M-PESA balance is Ksh(?<balance1>[\\d,]+\\.?\\d{0,2})\\. Transaction cost, Ksh(?<cost1>[\\d,]+\\.?\\d{0,2})\\.\\s*Amount you can transact within the day is (?<limit1>[\\d,]+\\.?\\d{0,2})(?<trailing1>.*)?|(?<txnId2>\\b[A-Z0-9]+\\b) Confirmed\\. You have sent Ksh(?<amount2>[\\d,]+\\.?\\d{0,2}) to (?<recipient2>[A-Za-z\\s]+) on (?<date2>\\d{2}/\\d{2}/\\d{4})\\s+at (?<time2>\\d{1,2}:\\d{2} [AP]M)\\. New MPESA balance is Ksh(?<balance2>[\\d,]+\\.?\\d{0,2})\\.".toRegex(
                RegexOption.IGNORE_CASE
            )
            ...
            RECEIVE_MONEY -> "(?:Congratulations!\\s+)?(\\b[A-Z0-9]+\\b)\\s+confirmed\\.\\s*You have received Ksh([\\d,]+\\.?\\d{0,2}) from ([A-Za-z0-9\\s\\p{P}\\p{S}_]+?)(?: (\\d{10}))? on (\\d{1,2}/\\d{1,2}/\\d{2,4})(?:\\s*at\\s*|\\s*)(\\d{1,2}:\\d{2} (?:AM|PM))\\.?\\s*New M-?PESA balance is Ksh([\\d,]+\\.?\\d{0,2})(.*)?".toRegex(
                RegexOption.IGNORE_CASE
            )
            ...
            BUNDLES_PURCHASE -> "(\\b[A-Z0-9]+\\b) Confirmed\\. Ksh([\\d,]+\\.?\\d{0,2}) sent to ([A-Z ]+) for account\\s*([A-Z ]+)\\s*on (\\d{1,2}/\\d{1,2}/\\d{2}) at (\\d{1,2}:\\d{2} [APM]{2})\\. New M-PESA balance is Ksh([\\d,]+\\.?\\d{0,2})\\. Transaction cost, Ksh([\\d,]+\\.?\\d{0,2})(.*)?"
                .toRegex(RegexOption.IGNORE_CASE)
            ...
            SEND_MONEY_FROM_POCHI -> "(\\b[A-Z0-9]+\\b) Confirmed. Ksh([\\d,]+\\.?\\d{0,2}) sent to ([A-Za-z0-9\\s\\p{P}\\p{S}_]+) on (\\d{1,2}/\\d{1,2}/\\d{2,4}) at (\\d{1,2}:\\d{2} [APM]{2})\\.? New business balance is Ksh([\\d,]+\\.?\\d{0,2})\\. Transaction cost, Ksh([\\d,]+\\.?\\d{0,2})\\. Amount you can transact within the day is ([\\d,]+\\.?\\d{0,2})(?:\\.(.*))?".toRegex(
                RegexOption.IGNORE_CASE
            )

            FULIZA_PAY -> FULIZA_CUT_REGEX

            REVERSAL_DEBIT -> "([A-Z0-9]+)\\s+confirmed\\. Reversal of transaction\\s+([A-Z0-9]+)\\s+has been successfully reversed\\s+on\\s+(\\d{1,2}/\\d{1,2}/\\d{2,4})\\s+at\\s+(\\d{1,2}:\\d{2}\\s+[APM]{2})\\s+and Ksh([\\d,]+\\.?\\d{0,2}) is debited from your M-PESA account\\.?\\s*New M-PESA account balance is Ksh([\\d,]+\\.?\\d{0,2})(.*)?".toRegex(
                RegexOption.IGNORE_CASE
            )

            REVERSAL_CREDIT -> "([A-Z0-9]+)\\s+confirmed\\. Reversal of transaction\\s+([A-Z0-9]+)\\s+has been successfully reversed\\s+on\\s+(\\d{1,2}/\\d{1,2}/\\d{2,4})\\s+at\\s+(\\d{1,2}:\\d{2}\\s+[APM]{2})\\s+and Ksh([\\d,]+\\.?\\d{0,2}) is credited to your M-PESA account\\.?\\s*New M-PESA account balance is Ksh([\\d,]+\\.?\\d{0,2})(.*)?".toRegex(
                RegexOption.IGNORE_CASE
            )
